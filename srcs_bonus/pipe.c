...
static void	add_pipe(float *vertices, int *index, float x1, float y1, float x2, float y2, float pipe_width){
	...
	float   vercteurDirecteurX = x2 - x1;
	float   vercteurDirecteurY = y2 - y1;

	float   length = sqrtf(vercteurDirecteurX * vercteurDirecteurX + vercteurDirecteurY * vercteurDirecteurY);
	if (length == 0.0f)
		return ;

	nx = -vercteurDirecteurY / length;
	ny = vercteurDirecteurX / length;
